...
def send_whatsapp_message(to, text):
    ...
    try:
        ...
        print(f"WhatsApp message sent successfully to {to}. Response: {response.status_code}")
        return True
...
def webhook():
    ...
    try:
        ...
        if order_id:
            id_val = order_id[0]
            n8n_webhook_url = os.getenv("N8N_WEBHOOK_URL", "https://n8n-setup-x4if.onrender.com/webhook-test/get-salla-order" )
            payload_to_n8n = {"order_id": id_val, "phone": sender, "message_text": msg_body}
            try:
